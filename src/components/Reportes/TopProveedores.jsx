...
export default function TopProveedores() {
  ...
  const obtenerDatos = () => {
    ...
    axios.get('http://localhost:6060/topproveedores')
      .then(res => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Top Proveedores");

    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });

    const dataBlob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(dataBlob, 'top_proveedores.xlsx');
  };

  const exportToPDF = () => {
    const input = document.getElementById('tabla-top-proveedores');
  ...
  return (
    <div className='container-fluid d-flex flex-column justify-content-center align-items-center bg-white'>
      <div className="d-flex flex-column justify-content-center align-items-center">
        <h2 className="mb-4 padding-top-25">Top 5 Proveedores</h2>
        <div className='w-100 bg-white rounded p-3'>
          <div className="table-responsive">
            <table className='table table-striped' id='tabla-top-proveedores'>
              <thead>
                <tr>
                  <th>Proveedor</th>
              ...
              <tbody>
                {proveedores.map((data, i) => (
                  <tr key={i}>
                    <td>{data.PRV_NOMBRE}</td>
                    <td>{data.TOTALENTRADAS}</td>
                  </tr>
                ))}
              </tbody>
            </table>
